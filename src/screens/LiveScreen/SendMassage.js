...
export const CompareObject = (props) => {
    if (props.objectFirst.Email == props.objectSecend.Email && props.objectFirst.Text == props.objectSecend.Text) {
        props.callback(true)
    } else { props.callback(false) }
}
export const AddDatas = (props) => {
    ...
    for (let index = 0; index < b2.length; index++) {
        ...
        CompareObject({
            objectFirst: element, objectSecend: element2, callback: (e) => {
                if (e) {
                    a1Index = index
                }
            }
...
export const SendTicket = async (props) => {
    if (props.Text != '') {
        let ID = await AsyncStorage.getItem('eventId')
        const Token = await AsyncStorage.getItem('Token');
        if (Token !== null && Token !== undefined && Token !== '') {
            request('POST', 'SendTicket', {
                ...
                (err) => { console.log(JSON.stringify(err)); props.callback(false) })
        } else {
...
export const MyTickets = async (props) => {
    // alert('dd')
    // console.log(props.HallName)
    let ID = await AsyncStorage.getItem('eventId')
    const Token = await AsyncStorage.getItem('Token');
    if (Token !== null && Token !== undefined && Token !== '') {
        request('POST', 'MyTickets', {
            HallName: props.HallName,
            EventId: ID,
            UserEmail: Token,
        }, () => { },
...
export default { SendTicket, MyTickets, CompareObject, AddDatas } 