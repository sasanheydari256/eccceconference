...
const EventsTime2 = (props) => {
  ...
  const addZero = (day) => {
    ...
    const myday =
      days[0] +
      '-' +
      (days[1].length > 1 ? days[1] : '0' + days[1]) +
      '-' +
      (days[2].length > 1 ? days[2] : '0' + days[2]);
    return myday;
  };
    ...
    request(
      ...
      (response) => {
        ...
        if (response.Result == 'Success' && response.Result != null) {
          ...
          // alert(JSON.stringify(response.Answer.DayList + ' ' + addZero(response.CurrentDate)));
          setDatas(a);
          setPrograms(response.Answer.DayList);
          const indexedayealan = response.Answer.DayList.indexOf(
            addZero(response.CurrentDate),
          );
          // alert(JSON.stringify(indexedayealan));
          getsesion(response.Answer, 0, indexedayealan, Word);
          setday(indexedayealan);
        ...
        <View style={Styles.searchview}>
          <View style={Styles.searchbox}>
            <TextInput
              ...
              style={Styles.txtsearch}></TextInput>
            <Icon
              onPress={() => Searches()}
              name={'search1'}
              type={'AntDesign'}
