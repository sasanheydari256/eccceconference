...
const SYDetaile = (props) => {
  ...
  const _downloadFile = (item) => {
    props.navigation.navigate('WSPDFreader', {Mydetailes: item});
  ...
  return (
    <View style={Styles.mainscrview}>
      ...
      <ScrollView
        ...
        <Text style={Styles.TxtHeder}>Media</Text>
        <View style={Styles.descview}>
          ...
          <TouchableOpacity
            onPress={() => _downloadFile(props.route.params.Datas)}
        ...
        <Text style={Styles.TxtHeder}>Program</Text>
        {props.route.params.Datas.WorkShopTime &&
          props.route.params.Datas.WorkShopTime.length > 0 &&
          props.route.params.Datas.WorkShopTime.map((e, index) => (
            <View
              // onPress={() => _downloadFile(props.route.params.Datas)}
              style={Styles.touchoricomm}>
              <View style={Styles.timeview}>
                <Text style={{color: '#FFFF'}}>{e.From}</Text>
                <Text style={{color: '#FFFF'}}>{e.Until}</Text>
              </View>
              <View style={Styles.programtxt}>
                <Text style={Styles.txtprodes}>{e.FullName}</Text>

                <Text style={Styles.txtproname}>{e.Title}</Text>
              </View>
            </View>
          ))}
...
export default connect(mapStateToProps, mapDispatchToProps)(SYDetaile);
